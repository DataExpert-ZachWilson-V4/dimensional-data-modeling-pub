...
changes AS (
    SELECT
        ...
        current_year,
  CASE
    WHEN did_change = 0 THEN ARRAY[CAST(ROW(COALESCE(CASE WHEN is_active_previous_year = 1 THEN 'true' ELSE 'false', is_active_current_year),COALESCE(quality_class_previous_year, quality_class_current_year),start_date,current_year) 

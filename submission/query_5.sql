-- Query that uses CTE to incrementally backfill actors history scd table with updated actors information
INSERT INTO amaliah21315.actors_history_scd 
WITH last_year_actors_scd AS (  -- Gets the data of actors from the previous years stored in local actors table
        SELECT *
        FROM amaliah21315.actors_history_scd
        WHERE current_year = 2010 -- previous reporting year
    ),
    this_year_actors AS (
        -- Selects actors, their IDs, film year, average ratings, and aggregates films into arrays
        SELECT *
        FROM amaliah21315.actors
        WHERE current_year = 2011 -- Consider data up to the year 2011
    ),
    combined_scd AS (
    ...
    changed AS (
        -- Formatting the changed records in an array column to unnest below
        SELECT actor,
            actor_id,
            current_year,
            CASE
